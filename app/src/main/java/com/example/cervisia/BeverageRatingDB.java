...
@Database(entities = {BeverageRating.class}, version = 4)
public abstract class BeverageRatingDB extends RoomDatabase {
    ...
    public static BeverageRatingDB getDatabaseInstance(Context context){
        if (INSTANCE == null){
            INSTANCE = Room.databaseBuilder(context.getApplicationContext(), BeverageRatingDB.class, "rating-database4").allowMainThreadQueries().build();
        }
        return INSTANCE;
    }
}
